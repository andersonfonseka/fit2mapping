...
public class RequirementSpecificationDiagram extends Diagram {
	
	public RequirementSpecificationDiagram(RequirementSpecificationDiagramEditor editor){
			...
			public void drop(DropTargetDropEvent dtde) {
				...
				while(shp.hasNext()){
					...
					if (shape instanceof LineAssociation){
						...
						getShapes().put(line.getId(), line);
					} else if (shape instanceof LineInclude){
						LineInclude line = ObjectCopyUtil.getInstance().getLineIncludeCopy(shape, getShapes());
						getShapes().put(line.getId(), line);
				...
				if ((getxPos() > elipse.getX() && getyPos() > elipse.getY())
					...
					if ((getObjectType().equals(ObjectEnum.Association)) 
							...
							if (getSelectedFigure() instanceof ScenarioFigure){
								setLineGenericSource((ScenarioFigure) getSelectedFigure());	
							}
					}
						...
						if (getLineGenericSource() != getSelectedFigure()){
							...
							if (getSelectedFigure() instanceof ScenarioFigure){
								...
								if (getLineGenericSource() instanceof RequirementFigure){
									((RequirementFigure) getLineGenericSource()).getRequirementBean().addScenarios(scenario.getScenarioBean());
